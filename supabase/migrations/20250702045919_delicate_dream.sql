/*
  # Create certification_details table with proper constraints

  This migration creates the certification_details table with proper constraints
  and avoids errors with existing constraints by using conditional checks.
  
  1. New Tables
    - certification_details: Stores detailed information about certifications
  
  2. Views
    - certifications_with_details: Joins certifications with their details
  
  3. Functions
    - get_certification_details: Returns formatted certification details
    - search_certifications_by_topic: Searches certifications by topic
    - insert_sample_certification_details: Helper function to insert sample data
*/
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_examination_details_not_empty'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_topics_is_array'
  ) THEN
    ALTER TABLE certification_details 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_evaluation_criteria_not_empty'
  ) THEN
    ALTER TABLE certification_details 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_prerequisites_is_array'
  ) THEN
    ALTER TABLE certification_details 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_learning_outcomes_is_array'
  ) THEN
    ALTER TABLE certification_details 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_update_certification_details_updated_at'
  ) THEN
